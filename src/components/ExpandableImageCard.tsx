...
import { CheckCircle, Clock, Copy, Download, Calendar, ChevronDown, ChevronUp, Info } from "lucide-react";
...
export const ExpandableImageCard = ({ image, expanded, onToggleExpand }: ExpandableImageCardProps) => {
  ...
  return (
    <Card className={`overflow-hidden transition-all ${expanded ? 'shadow-md' : 'hover:shadow-md'}`}>
          ...
          <div className="mt-6 border-t pt-4">
            <Accordion type="single" collapsible className="w-full">
                ...
                <AccordionContent className="pt-2 pb-3">
                  <div className="space-y-3 text-sm">
                    <div className="grid grid-cols-2 gap-y-2">
                      ...
                      <div>{image.maintainer}</div>
                      
                      <div className="text-muted-foreground">Support:</div>
                      <div>
                        {image.supportChannel.type} ({image.supportChannel.contact})
                      </div>
                      
                      <div className="text-muted-foreground">Security Scanned:</div>
                      <div>{image.securityScanned ? "Yes" : "No"}</div>
                      
                      <div className="text-muted-foreground">Docker Hub:</div>
                      <div className="truncate max-w-[200px]">{image.dockerHubUrl}</div>
                    </div>
                  </div>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="changelog" className="border-b-0">
                ...
                <AccordionContent className="pt-2 pb-3">
                  <div className="space-y-3 text-sm">
                    {image.changelog.map((log, index) => (
                      <div key={index} className="pb-2">
                        <div className="font-medium">
                          v{log.version} - {formatDate(log.date)}
                        </div>
              ...
              <AccordionItem value="usage" className="border-b-0">
                ...
                <AccordionContent className="pt-2 pb-3">
                  <div className="p-2 bg-gray-50 rounded-md font-mono text-xs border border-gray-200">
